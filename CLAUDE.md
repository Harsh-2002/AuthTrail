...
for Debian/Ubuntu + systemd + OpenSSH. It answers who accessed a server, from where, with which
...
`docs/security.md` for the security boundaries. `PRODUCT.md` defines current product intent and
user-visible behavior; `CONTRACT.md` defines the invariants every implementation must preserve.

Read `AGENTS.md` first for the required documentation order.

## Commands

```sh
make shellcheck     # shellcheck -s sh over POSIX components and -s bash over Bash-only hooks/completion
make test            # shellcheck + run every tests/test-*.sh
...
There is no build step — everything is POSIX `/bin/sh`, except the explicitly Bash-specific
...
- **Comments are single-line only** — no multi-line comment blocks/headers. Put longer design
  rationale in `docs/architecture.md` instead of inline.
...
won't catch (see `docs/operations.md`'s "Bugs found and fixed" section for concrete examples: a
